...
1. Hardware calibration (speed factor, pulses per meter, incline span, homing-related parameters)
2. Device preferences (WiFi settings, FTMS device name, debug mode, feature flags)
3. User profile settings (drag target defaults, pause target defaults, preset values)
4. Runtime-only state (raw counters, current treadmill state, current FTMS session state, current UI state)
...
**Compatibility and profile management**
Each treadmill profile should maintain:
- supported telemetry inputs and control paths
- confirmed homing and state decoding behavior
- forbidden commands and unresolvedprotocol anomalies
...
**Logging rules**
- timestamped and rate-limited.
- disableable at runtime.
- non-blocking to telemetry and state updates.
