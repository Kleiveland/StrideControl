...
Reason: Anchored to the measured 10 km/h reference point, which is closest to the primary operating range.

Derived odometer constant:

1 meter = 3.2292 pulses (derived from the chosen speed calibration).

Dynamic Speed Injection (AutoCal)
Because the treadmill's actual speed deviates from the requested target speed over time, the system uses a dynamic calibration offset table (stored in profiles.json).
The module uses linear interpolation to calculate a precise offset for any requested target speed based on nearest known neighbors.
Learning occurs passively: if the requested speed is stable for a set period, the system compares it against Pin 7 actual speed, updates the offset in RAM, and saves it to LittleFS only when the treadmill stops (to minimize flash wear).
...
The software does not hard-code a final pulses-per-percent incline constant yet. Instead:
...
The command-injection wiring must be designed so that loss of ESP32 power does not interrupt normal treadmill operation. The original console path must remain functional in the absence of ESP32 activity. This fail-safe behavior must be achieved by the surrounding wiring topology and must not be assumed from the SN74HC4066 alone.
...
First-boot behavior: On first boot, or whenever no valid WiFi credentials are available, the ESP32 shall start in Access Point mode. A local configuration page shall be exposed for entering WiFi credentials and core device settings.
...
Safety rule: Configuration mode must not interfere with treadmill telemetry capture once the device is already configured and operating normally.
...
Core 1: pulse counting interrupts for Pin 7 and Pin 11, real-time GPIO injection tasks, safety watchdog tasks

Thread Safety & Data Synchronization:
Because Core 0 (WebSockets/BLE) and Core 1 (Hardware/AutoCal) run simultaneously, any read or write access to the shared TreadmillData struct (globalState) MUST be protected by a global hardware spinlock (portMUX_TYPE). Direct manipulation of state variables without portENTER_CRITICAL and portEXIT_CRITICAL is strictly forbidden to prevent memory corruption and kernel panics.
...
Data path separation: Telemetry must pass through: hardware capture layer -> derived metrics layer -> internal state layer -> output adapters (WebSocket UI, FTMS, local logging).
...
Backpressure rule: If a downstream consumer cannot keep up, the newest valid state should replace the previous pending state. Stale telemetry frames may be dropped, but pulse counters, odometer, and homing-critical state must never be dropped.

Safety rule: No output layer may block the sensor capture layer.

8. CSAFE Rules (RS232)
Connection Interface: RJ45 to MAX3232
...
Inter-frame timing: A short inter-frame delay (e.g., 40 ms) may be used as a defensive firmware option.

Allowed and forbidden commands

Allowed: 0x80, 0x85

Forbidden: 0xAA (Extended Telemetry), 0xA5 (Speed), 0x9C (Error Codes). These corrupt the treadmill serial buffer.

Serial Fragmentation and Buffer Reassembly
The ESP32 UART receives fragmented packets. The code must assemble incoming bytes into a buffer, clear the buffer on 0xF1 (start frame), and only parse the state byte once 0xF2 (end frame) is received.

Internal CSAFE-Derived UI States
The firmware does not use raw CSAFE values directly as UI states. Instead, it derives the following internal states: STOPPED, PAUSED, STARTING, RUNNING, IDLE.
...
Data Integrity
The working CSAFE prototype did not validate the incoming checksum byte. Instead, frame validity was determined by frame boundaries and bit masking (state = raw_byte & 0x0F). Checksum mismatch is not currently treated as a hard reject condition.
...
Safety rule: Application code must never read directly from UART. All CSAFE input must pass through the framed parser and treadmill-specific translator.
...
Command Queuing (5-Second Rule): Because the treadmill hardware ignores speed/incline inputs until the belt is moving, UI commands issued while the internal state is STARTING must be placed in a queue. Once the state transitions to RUNNING, the queued commands are injected. If the treadmill does not reach RUNNING within 5 seconds of the user's input, the queued command expires.

10. Planned Command Injection Timing
Because the treadmill console only accepts whole numbers for instant speed and incline, decimal adjustments require a two-step macro sequence.
The current planned command-injection sequence is:

Isolate the decoded front-panel bus from the original console path.

Inject Base Value: Trigger Instant Speed/Incline -> [Whole Number digits] -> ENTER.

Inject Fine Tuning: Trigger Speed + or Speed - for the exact number of 0.1 increments calculated by the AutoCal logic.

Restore the original console path.

This timing model requires asynchronous queuing (a state machine without delay()) to provide the physical treadmill controller with enough inter-press time to register each "button press".
...
Primary UI purpose: It must support direct adjustment of treadmill speed/incline, display of actual speed/incline, treadmill state, and session time. Actual speed and actual incline must always be available in the web UI whenever the treadmill is operating.
...
Filesystem rules: HTML, CSS, JavaScript, and configuration files shall be stored in LittleFS. The LittleFS partition is built from a dedicated /data folder in the project workspace.

First-Boot Formatting: The storage manager must initialize the filesystem using LittleFS.begin(true). This ensures that a factory-new ESP32-S3 board automatically formats its partition table correctly on the very first boot without crashing.

Flash Wear Policy: The odometer and dynamically learned AutoCal offsets are only written to flash/LittleFS when the treadmill state changes to STOPPED or IDLE, minimizing write cycles.

12. Bluetooth FTMS Rules
BLE Stack: NimBLE-Arduino (chosen for lower memory footprint and stability).

Role: Dual-Role operation. The stack must simultaneously act as a BLE Client (HR Scanner) and a BLE Server (FTMS Broadcast).

Broadcast Rate: Exactly 1 Hz.

BLE / FTMS Connection Policy

The device shall advertise a stable and unique name.
...
Safety rule: Loss of a BLE client must not stop treadmill telemetry capture or corrupt treadmill state tracking.
...
Control Points: Fitness Machine Control Point requests from training apps must be acknowledged with valid ACK responses (using indicate()).

13. GUI Architecture and Frontend Logic
13.1 Layout Engine: The 5-Row Symmetrical Grid
To achieve perfect visual balance regardless of screen resolution, a fixed CSS Grid is utilized (grid-template-rows: 140px 1fr auto 1fr 140px;).
...
Tabular Numbers: All telemetry fonts must enforce the CSS rule font-variant-numeric: tabular-nums;.

Monochrome Heart Rate: The heart rate value must remain white. Colors are reserved for active commands.

13.3 Interaction Model (Zero-Lag)

Event Handling: Use pointerdown instead of click to eliminate mobile browser delay.

Haptic Feedback: Buttons must scale down to 0.92 on press.

13.4 Heart Rate Belt & Bluetooth LE (BLE) Discovery

The interface implements a discovery engine (Scan -> Select -> Connect), rather than a blind toggle.

13.5 Technical Frontend Stack

Vanilla JavaScript: Frameworks (React/Vue/etc.) are prohibited.

Communication: Bi-directional data exchange via a single WebSocket stream.

CSS Variable System: All colors and dimensions are defined as :root variables.

14. Current Open Items
Unmapped harness lines: Pin 3, Pin 4, Pin 5, Pin 8, Pin 9
...
Physical Console Key Synchronization: Listen for physical button activity using interrupts and debounce filtering.

Observability and Remote Debug: Expose structured logs for boot sequences, state transitions, FTMS events, and safety timeouts over the network.
...
FTMS via NimBLE (Dual-Role)

Command queuing logic defined for UI inputs during STARTING state

Global Mutex for thread safety across Core 0 and Core 1

LittleFS auto-formatting on first boot
