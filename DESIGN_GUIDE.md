...
Inter-frame timing:
The working browser prototype sent the 0x85 and 0x80 frames back-to-back with no enforced delay. A short inter-frame delay (for example around 40 ms) may still be used as a defensive firmware option if later testing shows buffer sensitivity on the treadmill controller.
...
State decoding
The treadmill state is extracted from the received status byte using: state = raw_byte & 0x0F
Observed raw state values: 5 = InUse, 6 = Paused, 8 = Starting, 0, 1, 2, 3, 15 = Stopped / Ready
Internal CSAFE-Derived UI States
The firmware does not use raw CSAFE values directly as UI states. Instead, it derives the following internal states:
STOPPED
PAUSED
STARTING
RUNNING
...
Scheduler rule
Only a controlled set of commands may be sent by the scheduler. The command layer must not allow random feature polling from multiple callers.
...
This timing model requires asynchronous queuing (a state machine without delay()) to provide the physical treadmill controller with enough inter-press time to register each "button press". It must be validated against the final decoded front-panel bus implementation.
...
The LittleFS partition is built from a dedicated /data folder in the project workspace containing all frontend assets (e.g., index.html, style.css, app.js) and the profiles.json storage file. This allows frontend updates without compiling C++ firmware.
...
Role: Dual-Role operation is required. The stack must simultaneously act as a BLE Client (to discover and read the user's HR belt) and a BLE Server (to broadcast FTMS data to Zwift/Garmin).
...
Workflow:
User initiates "SCAN FOR DEVICES".
ESP32 returns a JSON list of available BLE device names and IDs.
UI populates a selection list (e.g., "Polar H10", "Garmin HRM").
Connection status is persisted in the ESP32 NVS/LittleFS for automatic reconnection in future sessions.
